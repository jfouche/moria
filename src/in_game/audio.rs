...
pub fn plugin(app: &mut App) {
    app.insert_resource(AudioVolume(9))
        .add_systems(OnEnter(GameState::Game), start_music)
        .add_systems(Update, change_volume.run_if(in_state(GameState::Game)))
        .add_systems(OnExit(GameState::Game), despawn_all::<MyMusic>);
}
